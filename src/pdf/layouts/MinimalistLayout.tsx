...
export function MinimalistPdfLayout({
  data,
  theme,
  scale,
  sectionOrder,
  ...
  const sectionsMap: Record<string, React.ReactNode> = {
    summary: data.summary?.trim() ? (
      <View style={{ marginBottom: t(12, scale) }}>
        <PdfAccentBarHeading title="Professional Summary" scale={scale} theme={theme} fontFamily={ff} variant="minimalist" />
        <PdfText style={{ fontSize: t(10, scale), lineHeight: 1.4, color: theme.textColor, fontFamily: ff }}>{data.summary}</PdfText>
      </View>
    ) : null,
    experience:
      experience.length > 0 ? (
        <View style={{ marginBottom: t(12, scale) }}>
          <PdfAccentBarHeading title="Experience" scale={scale} theme={theme} fontFamily={ff} variant="minimalist" />
          {experience.map((exp) => (
            <View key={exp.id} style={{ marginBottom: t(8, scale) }}>
              ...
              />
              {exp.location ? (
                <PdfText style={{ fontSize: t(8.5, scale), color: '#a1a1aa', ...italic, fontFamily: ff, marginTop: t(1, scale) }}>
                  {exp.location}
                </PdfText>
              ) : null}
    ...
    projects:
      projects.length > 0 ? (
        <View style={{ marginBottom: t(12, scale) }}>
          <PdfAccentBarHeading title="Projects" scale={scale} theme={theme} fontFamily={ff} variant="minimalist" />
          {projects.map((proj) => (
            <View key={proj.id} style={{ marginBottom: t(8, scale) }}>
              ...
              />
              <PdfText style={{ marginTop: t(2, scale), fontSize: t(10, scale), lineHeight: 1.35, color: theme.textColor, fontFamily: ff }}>
                {proj.description}
              </PdfText>
            </View>
          ))}
    ...
    education:
      education.length > 0 ? (
        <View style={{ marginBottom: t(12, scale) }}>
          <PdfAccentBarHeading title="Education" scale={scale} theme={theme} fontFamily={ff} variant="minimalist" />
          {education.map((edu) => (
            <View key={edu.id} style={{ marginBottom: t(8, scale) }}>
              ...
              />
              <PdfText style={{ fontSize: t(9.5, scale), color: theme.mutedColor, marginTop: t(1, scale), fontFamily: ff }}>
                {edu.degree}
    ...
    skills:
      skills.length > 0 ? (
        <View style={{ marginBottom: t(12, scale) }}>
          <PdfAccentBarHeading title="Skills" scale={scale} theme={theme} fontFamily={ff} variant="minimalist" />
          <PdfText style={{ fontSize: t(10, scale), lineHeight: 1.35, color: theme.textColor, fontFamily: ff }}>
      ...
      <View style={{ marginBottom: t(15, scale), borderBottomWidth: 2, borderBottomColor: theme.primaryColor, paddingBottom: t(8, scale) }}>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: t(4, scale) }}>
          {[
            ...
            .filter(Boolean)
            .map((item) => (
              <PdfText key={String(item)} style={{ fontSize: t(9.5, scale), color: theme.mutedColor, marginRight: t(8, scale), textTransform: 'uppercase' }}>
                {item}
              </PdfText>
            ))}
        </View>
