import type { ResumeData, AtsScore, SkillsMatrixItem } from '../types/resume'
...
export function calculateSkillsMatrix(resume: ResumeData, jobDescription: string): SkillsMatrixItem[] {
  ...
  return SKILL_CATEGORIES_CONFIG.map(cat => {
    ...
    if (jdWords.length > 0) {
      ...
      // Candidate score is the percentage of matches compared to what's required
      candidateScore = Math.round((matched.length / jdWords.length) * requiredScore)
