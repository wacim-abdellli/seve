...
const ProfessionalTemplate = memo(function ProfessionalTemplate({
  data, activeSection, atsMode, onEditSection,
  sectionOrder = ['summary', 'experience', 'projects', 'education', 'skills'],
  onDragStart, onDragOver, onDrop, themeColor = '#1e293b'
          ...
          {education.map((edu) => (
            <div key={edu.id} className="edu-entry">
              <div className="flex justify-between items-baseline">
                ...
                <span className="text-[9px] text-slate-500">{formatDate(edu.graduationDate)}</span>
              </div>
              <div className="text-[9.5px] text-slate-600">{edu.degree}{edu.gpa && parseFloat(edu.gpa) >= 3.5 ? ` — GPA: ${edu.gpa}` : ''}</div>
            </div>
          ))}
        </div>
      </div>
          ...
          {projects.map((proj) => (
            <div key={proj.id} className="proj-entry">
              <div className="flex justify-between items-baseline">
                ...
                <span className="text-[8.5px] text-slate-500">{proj.technologies.join(', ')}</span>
              </div>
        ...
        <div className="space-y-1.5">
          {awards.map((a) => (
            <div key={a.id} className="flex justify-between items-baseline">
              <span className="text-[9.5px] text-slate-700">{a.title}{a.awarder ? ` — ${a.awarder}` : ''}</span>
              {a.date && <span className="text-[8.5px] text-slate-500">{a.date}</span>}
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="space-y-1.5">
          {certifications.map((c) => (
            <div key={c.id} className="flex justify-between items-baseline">
              <span className="text-[9.5px] text-slate-700">{c.title}{c.issuer ? ` — ${c.issuer}` : ''}</span>
              {c.date && <span className="text-[8.5px] text-slate-500">{c.date}</span>}
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="resume-template" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      <div className="resume-page text-[9.5px] leading-normal text-slate-800 p-10 select-text max-w-full space-y-4" style={{ paddingTop: 48, paddingBottom: 48 }}>
        ...
        >
          <div className="mb-4 pb-3 border-b-2" style={{ borderBottomColor: themeColor }}>
            <h1 className="text-2xl font-extrabold tracking-tight" style={{ color: themeColor }}>{getFullName(contact) || 'YOUR NAME'}</h1>
      ...
      {sectionData.page2Sections.length > 0 && (
        <>
          ...
          <div className="resume-page resume-page-continuation text-[9.5px] leading-normal text-slate-800 p-10 select-text max-w-full space-y-4" style={{ paddingTop: 48, paddingBottom: 48, fontFamily: "'Inter', system-ui, sans-serif" }}>
