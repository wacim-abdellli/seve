...
import {
  ...
  Eye, EyeOff, ExternalLink, Zap, Shield, Trash2
} from 'lucide-react'
...
export default function AiSettingsModal({ onClose }: AiSettingsModalProps) {
  ...
  const [selectedProvider, setSelectedProvider] = useState<AiProvider>(config?.provider ?? 'groq')
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-white/5">
          <div>
            <h2 id="ai-settings-heading" className="text-sm font-bold text-white">AI Assistant Setup</h2>
            <p className="text-[11px] text-zinc-500 mt-0.5">Connect a free AI key to unlock smart features</p>
          </div>
          <button
            onClick={onClose}
            className="w-7 h-7 rounded-lg flex items-center justify-center text-zinc-500 hover:text-white hover:bg-white/5 transition-colors cursor-pointer"
            aria-label="Close"
          >
            <X className="w-4 h-4" />
          </button>
        ...
        <div className="p-5 space-y-5">
          {/* Step 1 — Provider */}
          <div>
            <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-2.5">1 · Choose Provider (all free)</p>
          ...
          {/* Step 2 — API Key */}
          <div>
            <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-2.5">2 · Paste API Key</p>
          ...
          {/* Step 3 — Test */}
          <div>
            <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-2.5">3 · Test & Save</p>
            <button
              onClick={handleTest}
              disabled={!apiKey.trim() || testStatus === 'testing'}
              className={`w-full h-9 rounded-xl flex items-center justify-center gap-2 text-xs font-bold transition-all border cursor-pointer mb-2 ${
            ...
            <AnimatePresence>
              {testMessage && (
                <motion.div
                  ...
                  className={`flex items-center gap-2 text-[11px] font-medium px-3 py-2 rounded-lg mb-3 ${
                    testStatus === 'success'
            ...
            <div className="flex items-start gap-2 rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
              ...
              <p className="text-[10px] text-zinc-600 leading-relaxed">
                Key stored <span className="text-zinc-400">only in your browser</span>. Never sent to Seve's servers.
              </p>
            </div>
          </div>
        </div>
